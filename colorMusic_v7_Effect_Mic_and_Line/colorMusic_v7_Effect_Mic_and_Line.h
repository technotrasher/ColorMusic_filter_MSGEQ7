...
void setPixelHeatColor (int Pixel, byte heatramp) {
...
void Fire(int Cooling, int Sparking) {
  if (timer_func(mainSettings.COLOR_SPEED - 100)) {
    ...
    for ( int k = (SPLIT_NUM_LEDS) - 1; k >= 2; k--) {
    ...
    for ( int j = 0; j < SPLIT_NUM_LEDS; j++) {
...
void brights_dec() {
  if (timer_func()) {
    ...
    for (byte i = 0; i < NUM_LEDS / NM; i++) { //уменьшаем яркость всех горящих пикселей на 1
      if (heat[i] != 0) {
        heat[i] = heat[i] - 1;
        for (byte b = 0; b < NM; b++) leds[i * NM + b] = CHSV(heat[SPLIT_NUM_LEDS + i], thissat, heat[i]);
      }
    }
  }
}
