void animation() {
  ...
  switch (mainSettings.this_mode) {
        ...
        switch (mainSettings.cmu2_mode) {
          ...
          case 2: // вспышки бегут2...
            if (timer_func(mainSettings.FLASH_PERIOD)) {
              ...
              for (byte i = 0; i < P_SPEED; i++) leds[i] = color;
              //}
            }
            break;
        }
    ...
    case 7: {
        switch (mainSettings.freq_strobe_mode) {
          case 0:
            ...
            if (timer_func(mainSettings.RUNNING_SPEED)) {
              for (int i = 0; i < SPLIT_NUM_LEDS - 1; i++) {
                leds[i] = leds[i + 1];
    ...
    case 8: {
        ...
        for (int i = 0; i < SPLIT_NUM_LEDS; i++) {
          byte this_bright_R = map(freq_f_R[(int)floor((SPLIT_NUM_LEDS - i) / freq_to_stripe)], 0, freq_max_f_R, 0, 255);
          byte this_bright_L = map(freq_f_L[(int)floor((SPLIT_NUM_LEDS - i) / freq_to_stripe)], 0, freq_max_f_L, 0, 255);
