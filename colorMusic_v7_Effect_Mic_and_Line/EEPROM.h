//-----------------------------------Запись в EEPROM--------------------------------------------
/*
void EEPROM_WriteBlock(uint8_t addr, uint16_t data, uint8_t sizeData) //запись блока в EEPROM
{
...
void EEPROM_ReadBlock(uint8_t addr, uint16_t data, uint8_t sizeData) //чтение блока из EEPROM
{
  for (uint8_t i = 0; i < sizeData; i++) { //читаем побайтно блок данных
    ...
    EEAR = addr + i; //загружаем адрес
    EECR |= (0x01 << EERE); //запускаем чтение
    *(uint16_t*)(data + i) = EEDR; //записываем байт в блок памяти
  }
}
/*
//-----------------------------------Запись байта в EEPROM--------------------------------------------
void EEPROM_WriteByte(uint8_t addr, uint8_t data) //запись байта в EEPROM
{
  while (EECR & (0x01 << EEPE)); //ждем окончания записи
...
void EEPROM_UpdateByte(uint8_t addr, uint8_t data) //обновление байта в EEPROM
{
  while (EECR & (0x01 << EEPE)); //ждем окончания записи
  EEAR = addr; //загружаем адрес
  EECR |= (0x01 << EERE); //запускаем чтение
...
uint8_t EEPROM_ReadByte(uint8_t addr) //чтение байта из EEPROM
{
  while (EECR & (0x01 << EEPE)); //ждем окончания записи
  EEAR = addr; //загружаем адрес
  EECR |= (0x01 << EERE); //запускаем чтение
