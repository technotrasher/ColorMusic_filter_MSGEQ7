...
// раскомментировать для определения кодов пульта
//--------------------------------------------------
    //if (IRdata != 292984781) { // Это число выдаёт китайский WAVGAT пульт, вместе с полезной информацией, при нажатии на любую кнопку
	      //Serial.print("0x");
    ...
    if ((IRdata) == BUTT_STAR) {
      ...
      if (settings_mode == true) {
        EEPROM_UpdateByte(0, 0);
        resetFunc();  // сброс и запись 0 в ячейку 0.
      }
      Proc_OnOff();  // включить - выключить выполнение программы
    }

    if (ONstate == false) {
      eeprom_flag = false;  // мы принесли вам костыли!
      goto TickExit;
    }
    switch (IRdata) {  //режимы и подрежимы
      ...
      case BUTT_HASH:
        ...
        switch (mainSettings.this_mode) {
          case 0:
            if (++mainSettings.cmu_mode > 2) mainSettings.cmu_mode = 0;
            break;
          case 1:
            reset_arr();
            if (++mainSettings.cmu1_mode > 3) mainSettings.cmu1_mode = 0;
            break;
          case 2:
            if (++mainSettings.cmu2_mode > 2) mainSettings.cmu2_mode = 0;
            break;
          case 3:
            if (++mainSettings.cmu3_mode > 2) mainSettings.cmu3_mode = 0;
            break;
          case 5:
            mainSettings.LIGHT_COLOR = mainSettings.LIGHT_COLOR + 20;
            if (mainSettings.LIGHT_COLOR > 255) mainSettings.LIGHT_COLOR = 0;
            break;
          case 6:
            if (++mainSettings.light_mode > 1) mainSettings.light_mode = 0;
            break;
          case 7:
            if (++mainSettings.freq_strobe_mode > 1) mainSettings.freq_strobe_mode = 0;
            break;
          case 9:
            if (++mainSettings.effect_mode > 9) mainSettings.effect_mode = 0;
            one_color_all();
            break;
        }
        break;
      ...
      case BUTT_2:
        buttonTickLed();
        reset_arr();
        mainSettings.this_mode = 1;
        break;
      case BUTT_3:
        buttonTickLed();
        mainSettings.this_mode = 2;
        break;
      case BUTT_4:
        buttonTickLed();
        mainSettings.this_mode = 3;
        break;
      case BUTT_5:
        buttonTickLed();
        mainSettings.this_mode = 4;
        break;
      case BUTT_6:
        buttonTickLed();
        mainSettings.this_mode = 5;
        break;
      case BUTT_7:
        buttonTickLed();
        mainSettings.this_mode = 6;
        break;
      case BUTT_8:
        buttonTickLed();
        mainSettings.this_mode = 7;
        break;
      ...
      case BUTT_0:
        buttonTickLed();
        if (settings_mode) {
          if (line_mode) {      // сейчас подключен линейный вход
            line_mode = false;  // переключаемся на микрофон
            PORTB |= (1 << PB1);
            cli();
            a_channel = 2;                     // признак - канал микрофона
            channels[0]->offChannel();         // выключаем канал L
            channels[1]->offChannel();         // выключаем канал R
            channels[2]->onChannel();          // включаем микрофон
            buffer = channels[2]->getBands();  // переключаем буфер
            sei();
            _delay_ms(20);
          } else {             // сейчас подключен микрофон
            line_mode = true;  // переключаемся на линейный вход
            PORTB &= ~(1 << PB1);
            cli();
            a_channel = 0;                     // признак - канал L
            channels[0]->onChannel();          // включаем канал L
            channels[1]->onChannel();          // включаем канал R
            channels[2]->offChannel();         // выключаем микрофон
            buffer = channels[0]->getBands();  // переключаем буфер
            sei();
            _delay_ms(20);
          }
      ...
      case BUTT_UP:
        ...
        if (settings_mode) {
          ...
          effect_delay = smartIncr(effect_delay, 10, 0, 200);
        } else {
          switch (mainSettings.this_mode) {
      ...
      case BUTT_LEFT:
        ...
        if (settings_mode) {
          ...
          switch (mainSettings.this_mode) {
            case 0:
            case 1: mainSettings.SMOOTH = smartIncrFloat(mainSettings.SMOOTH, -0.05, 0.05, 1); break;
      ...
      case BUTT_RIGHT:
        ...
        break;
      default: eeprom_flag = false; break;  // если не распознали кнопку, не обновляем настройки!
    }
TickExit:
    ir_flag = false;
