...
/*
  ...
  найти в скетче строки Serial.println(IRdata, HEX); в функции remoteTick()
...
// лента
#define NUM_LEDS 288                       // количество светодиодов 180 - 1 метр
const byte SPLIT_NUM_LEDS = NUM_LEDS / 2;  // половина ленты
#define CURRENT_LIMIT 3500                 // лимит по току в МИЛЛИАМПЕРАХ, автоматически управляет яркостью (пожалей свой блок питания!) 0 - выключить лимит
...
const float MAX_COEF_FREQ_1[7] = {0.75, 0.9, 1.05, 1.2, 1.25, 1.35, 1.5};  // отдельные коэффициенты для впышек по частотам (0.75, 0.9, 1.05, 1.2, 1.25, 1.35, 1.5)
...
#define PEAK_FALL 1      // скорость падения пика 0-10
