...
void setup() {
  ...
  //analogReference(EXTERNAL); 1V
  // http://www.fi-com.ru/mcu/avr-reg/ADMUX.htm
  //Регистр ADMUX задаёт входной контакт порта A для подключения АЦП, выравнивание результата и выбор опорной частоты. 
  //Если бит ADLAR установлен, то результат преобразования размещается в регистровой паре ADCH-ADCL с выравниванием влево, в противном случае - с выравниванием вправо.
  //Опорная частота от внутреннего генератора задаётся выставленными в 1 битами REFS1 и REFS0. Если оба бита сброшены, то опорная частота берётся от контакта AREF. 
  ...
  /*
  По поводу референса. Он, действительно, используется внешний и является переменным. Потенциал шины референса определяется 4 источниками:
 1. 1В от блока питания - это минимальный потенциал, который может быть на входе ref согласно datasheet.
 2. Сигнал с выходов ОУ всех полос, объединенный с помощью диодов. Если со всех полос сигнал меньше 1В (см. п. 1), то диоды закрыты напряжением п.1 и сигнал ни с одной полосы не поступает на ref и там, соответственно, 1В. Если сигнал больше 1В, то с полосы с наибольшей амплитудой напряжение закрывает все остальные источники.
 3. D7, который позволяет уменьшить напряжение на ref, если нужно сделать это быстрее, чем определенно конденсатором на ref и оперативно повысить чувствительность схемы.
 4. D8, который позволяет увеличить напряжение на ref, если код определил, что происходит перегрузка по входу и нужно уменьшить чувствительность схемы.
  */
